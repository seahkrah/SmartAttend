...
export const SkeletonBlock: React.FC<{ width?: string; height?: string; className?: string }> = ({
  ...
  <div
    className={`${width} ${height} rounded bg-sunken animate-pulse ${className}`}
  />
);
...
export const CardSkeleton: React.FC = () => (
  <div className="p-6 bg-sunken rounded-lg border border-subtle space-y-4">
    ...
    <div className="flex flex-col items-center">
      <svg width={sizeMap[size]} height={sizeMap[size]}>
        <circle
          ...
          strokeWidth={4}
          fill="none"
          className="text-[rgb(var(--border-subtle))]"
        />
        <circle
          cx={sizeMap[size] / 2}
          cy={sizeMap[size] / 2}
    ...
    {steps.map((step, index) => (
      <React.Fragment key={index}>
        ...
        <div className="flex flex-col items-center">
          ...
          <p
            className={`text-xs mt-2 text-center ${
              step.status === 'current' ? 'text-blue-700 dark:text-blue-400 font-semibold' : 'text-secondary'
            }`}
          >
            {step.label}
          </p>
        ...
        {index < steps.length - 1 && (
          <div className="flex-1 h-0.5 mb-8 mx-1 bg-sunken" />
        )}
      </React.Fragment>
    ))}
  </div>
...
export const LoadingButton: React.FC<{
  ...
  const variantClasses = {
    ...
    secondary: 'bg-raised hover:bg-sunken text-primary',
...
export const LoadingInput: React.FC<{
  ...
  <input
    ...
    className="w-full px-3 py-2 bg-sunken border border-strong rounded text-primary placeholder:text-muted disabled:opacity-50 disabled:cursor-not-allowed"
  />
);
