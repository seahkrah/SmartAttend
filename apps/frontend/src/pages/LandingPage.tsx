...
const ProductCard: React.FC<{
  ...
  <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 sm:p-8 h-full">
    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-5 ${tone === 'brand' ? 'bg-brand-500/20 text-brand-300' : 'bg-accent-500/20 text-accent-300'}`}>
      <Icon className="w-6 h-6" aria-hidden />
    </div>
    <p className={`text-xs font-semibold uppercase tracking-widest ${tone === 'brand' ? 'text-brand-300' : 'text-accent-300'}`}>{eyebrow}</p>
    <h3 className="text-2xl font-bold mt-1 mb-5">{title}</h3>
    <ul className="space-y-3">
      {items.map((item) => (
        <li key={item} className="flex gap-3 text-slate-300">
          <Check className={`w-5 h-5 flex-shrink-0 mt-0.5 ${tone === 'brand' ? 'text-brand-300' : 'text-accent-300'}`} aria-hidden />
          <span>{item}</span>
        </li>
      ))}
    </ul>
    ...
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#070a14]/80 backdrop-blur-xl">
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4" aria-label="Site">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <a href="#platforms" className="hidden sm:inline text-sm text-slate-300 hover:text-white">Platforms</a>
          <Link to="/login" className="whitespace-nowrap text-sm text-slate-300 hover:text-white px-2 py-1">Sign in</Link>
    ...
    <main className="relative">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-16 text-center">
        <FadeIn>
          ...
          />
          <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] text-accent-300">{BRAND_TAGLINE}</p>
          ...
          <p className="mt-6 text-sm text-slate-400">
            ...
            <Link to="/register" className="text-accent-300 hover:text-accent-200 underline-offset-4 hover:underline">Request access</Link>
          </p>
        </FadeIn>
      </section>
          ...
          {FOUNDATIONS.map(({ icon: Icon, title, body }, i) => (
            <FadeIn key={title} delay={0.05 * i}>
              <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 h-full">
                <Icon className="w-6 h-6 text-accent-300 mb-3" aria-hidden />
